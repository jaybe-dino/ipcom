...
- **P0 검증** — 데이터 모델 확정, 인터랙티브 프로토타입(목업 → 웹) ✅
- **P1 MVP** — 단일 IP 스페이스 + 이미지/영상 플러그인 + Consent Matrix + 반출/정산 ✅
  - 도메인·게이트·원장·반출/정산 ✅ / 실시간·인증·DB·실모더레이션 ✅
- **P2 마켓 / P3 확장** — 마켓플레이스 ✅, 다플러그인 ✅, 계보/감사/포털 ✅ / 다IP·B2B·블록체인 원장 (예정)

## 7. 구현 완료(P1~P2)

1. 인증/권한(JWT + RBAC) — `x-user-id` 스텁 대체 ✅
2. `Store` → 영속 DB(Drizzle + Postgres/PGlite) + 마이그레이션 ✅
3. 실시간 채널(WebSocket) — Community Service ✅
4. NIM 어댑터 실연동 + 음악/보이스/3D + Higgsfield 멀티 어댑터 + 모델 선택 ✅
5. 실제 픽셀 워터마크(PNG LSB) + 에셋 스토리지/CDN(S3 SigV4) ✅
6. `packages/client-core` 추출 → 웹/데스크탑(Tauri)/모바일(Expo) 셸 ✅
7. 마켓플레이스(리스팅·주문·take rate) + 프로모션 쿠폰 + 바이어/판매자 포털 ✅
8. 라이선스 유효기간 + 만료 임박 알림, 리믹스 계보(Lineage) 뷰어 ✅
9. 정산 대시보드 차트 + 감사 콘솔(원장 무결성·신고 큐) + 원장/정산 CSV 내보내기 ✅
10. 아웃바운드 알림(웹훅/이메일·푸시) + 인앱 알림 통합(멘션·반출·정산·만료) ✅

## 8. 다음 작업(예정)

- 다중 IP·B2B 라이선싱, 블록체인/NFT 원장 임베딩, 결제 PG 실연동(Stripe 활성화)
